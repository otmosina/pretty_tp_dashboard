  ...
  Constant.graph_count_columns.downto(0) do |hour_ago|
    ...
    3.times.each do |s_index|
      y = (Report.wl_searches( hour_ago+Constant.time_shifts[s_index] )[0] rescue 0)
      series.add_point( s_index, i, y )
    end
    i += 1
  end

  last_hour = Report.wl_searches( 0, report_name )[1] rescue Time.now.utc.hour
  ...
  SCHEDULER.every '10s' do
    ...
    3.times.each do |s_index|
      y = (Report.wl_searches(Constant.time_shifts[s_index] )[0] rescue 0)
      series.add_point( s_index, last_x, y)
    end
    puts series.get.to_s
    #send_event("wl_searches", series: series.get)
    send_event("wl_searches", {
