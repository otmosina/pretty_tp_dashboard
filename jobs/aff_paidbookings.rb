  ...
  Constant.graph_count_columns.downto(0) do |hour_ago|
    ...
    3.times.each do |s_index|
      y = (Report.aff_paidbookings( hour_ago+Constant.time_shifts[s_index] )[0] rescue 0)
      series.add_point( s_index, i, y )
    end
    i += 1
  end

  last_hour = Report.aff_paidbookings( 0, report_name )[1] rescue Time.now.utc.hour
  ...
  SCHEDULER.every '10s' do
    ...
    3.times.each do |s_index|
      y = (Report.aff_paidbookings(Constant.time_shifts[s_index] )[0] rescue 0)
      series.add_point( s_index, last_x, y)
    end
    puts series.get.to_s
    #send_event("aff_paidbookings", series: series.get)
    send_event("aff_paidbookings", {
